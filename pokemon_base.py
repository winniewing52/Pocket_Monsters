...
class TypeEffectiveness:
    ...
    Attributes:
        EFFECT_TABLE: A 2D array which represents the type effectiveness value.
    """
    EFFECT_TABLE = ArrayR(len(PokeType))

    with open ('type_effectiveness.csv', 'r') as file:
        first_line = file.readline()
        for i in range (len(PokeType)):
            line = file.readline().split(',')
            EFFECT_TABLE[i] = ArrayR(len(PokeType))
            for j in range (len(PokeType)):
                EFFECT_TABLE[i][j] = float(line[j])
...
class Pokemon(ABC):
    ...
    def attack(self, other_pokemon) -> float:
        ...
        """
        type_effectiveness = TypeEffectiveness.get_effectiveness(self.get_poketype(), other_pokemon.get_poketype())

        attack_power = self.get_battle_power()
        other_pokemon_defend = other_pokemon.get_defence()

        if other_pokemon_defend < (attack_power / 2):
            first_attack = attack_power - other_pokemon_defend

        elif other_pokemon_defend < attack_power:
            first_attack = math.ceil(attack_power * 5/8 - other_pokemon_defend / 4)
        
        else:
            first_attack = math.ceil(attack_power / 4)

        return first_attack * type_effectiveness
    ...
    def _evolve(self) -> None:
        """
        Evolves the Pokemon to the next stage in its evolution line, and updates its attributes accordingly.

        Time Complexity Analhysis:
            Best Case: O(1), occurs when the Pokemon has no evolution line.
            Worst Case: O(1), occurs when the Pokemon has and evolution line, and the method performs the evolution by removing
                        the first element from the evolution line and update the attributes.
        """
        if self.evolution_line:
            self.evolution_line.pop(0)
            if self.evolution_line:
                self.name = self.evolution_line[0]
